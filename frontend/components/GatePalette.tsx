  ...
  return (
    <button
      ...
      style={{
        borderRadius: 10,
        padding: "9px 4px 7px",
        border: `1.5px solid ${isSelected ? color : hov ? `${color}44` : T.border}`,
        background: isSelected ? `${color}18` : hov ? `${color}0e` : `${color}07`,
        cursor: "pointer",
...
export const GatePalette = memo(function GatePalette({
  ...
  const hoverPreview = useMemo(() => {
    ...
    return isParametricGate(hoveredGate.type) ? `${hoveredGate.description} · theta ${formatTheta(theta)}` : hoveredGate.description;
  }, [hoveredGate, theta]);
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 6 }}>
                {gates.map((def) => (
                  <GateButton
                    ...
                    onSelect={() => onSelect(selected === def.type ? null : def.type)}
                  />
                ))}
              </div>
            )}
